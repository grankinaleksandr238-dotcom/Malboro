...
async def buy_callback(callback: types.CallbackQuery):
    ...
    try:
        async with aiosqlite.connect(DB_PATH, timeout=10) as db:
            ...
            name = item[0] if item[0] is not None else "Товар"
            price = item[1] if item[1] is not None else 0
            category = item[2] if item[2] is not None else 'gift'
            effect = item[3] if item[3] is not None else None
            stock = item[4] if item[4] is not None else -1
