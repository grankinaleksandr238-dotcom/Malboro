...
SUPER_ADMINS = [8127013147]  # замените на свой ID
DB_PATH = 'database.db'

# Лимиты для воровства
...
async def create_default_items():
    default_items = [
        ...
        ("🔍 Детектив", "Показывает, кто ограбил тебя в последний раз", 50, 'crime', 'detective', -1),
    ]
    async with aiosqlite.connect(DB_PATH) as db:
...
async def add_to_inventory(user_id: int, item_id: int, quantity: int = 1, uses_from_item: int = -1):
    ...
    - Если предмет с uses_left (например, защита), то увеличивается uses_left на uses_from_item.
    - Если предмет без uses (детектив), увеличивается quantity.
    """
    async with aiosqlite.connect(DB_PATH) as db:
...
async def remove_from_inventory(user_id: int, item_id: int, quantity: int = 1, uses: int = 0):
    ...
    - Если указаны uses > 0, то уменьшаются uses_left.
    - Иначе уменьшается quantity.
    При достижении 0 запись удаляется.
    """
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("SELECT quantity, uses_left FROM inventory WHERE user_id=? AND item_id=?", (user_id, item_id))
        row = await cur.fetchone()
...
async def has_item(user_id: int, item_id: int, need_uses: int = 1) -> bool:
    ...
    Для предметов с uses_left проверяет, что uses_left >= need_uses.
    Для обычных – что quantity >= 1.
    """
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("SELECT quantity, uses_left FROM inventory WHERE user_id=? AND item_id=?", (user_id, item_id))
        row = await cur.fetchone()
...
async def get_item_by_effect(effect_prefix: str):
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("SELECT id, effect FROM shop_items WHERE effect LIKE ?", (effect_prefix + '%',))
        row = await cur.fetchone()
...
async def check_ban(message: types.Message, state: FSMContext):
    ...
    if await is_banned(user_id):
        await message.answer("⛔ Вы заблокированы в боте. Обратитесь к администратору.")
        return
...
async def theft_target(message: types.Message, state: FSMContext):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        # Проверяем наличие ловушки у жертвы
        ...
        # Проверяем наличие защиты у жертвы
        ...
        # Базовая вероятность успеха 40%
        chance = 40
        # Отмычка повышает шанс
        tool_item_id = await get_item_by_effect('tool+')
        if tool_item_id and await has_item(robber_id, tool_item_id):
            ...
            if tool_uses > 0:
                chance += 20
                # Используем отмычку (1 использование)
                await remove_from_inventory(robber_id, tool_item_id, uses=1)

        # Защита понижает шанс
        if protect_uses > 0:
            chance -= 20
            # Уменьшаем uses_left защиты на 1
        ...
        if trap_uses > 0:
            ...
            # Ловушка тратит одно использование
            await remove_from_inventory(target_id, trap_item_id, uses=1)
            # Записываем статистику (попытка была, но украдено 0, так как грабитель потерял)
            await update_theft_stats(robber_id, target_id, 0)
            await db.commit()
            await state.finish()
            return

        # Если ловушки нет или она не сработала, продолжаем
        if success:
            steal_amount = random.randint(1, min(MAX_STEAL_AMOUNT, victim_balance))
            ...
            await db.execute("UPDATE users SET balance = balance - ? WHERE user_id=?", (steal_amount, target_id))
            await db.execute("UPDATE users SET balance = balance + ? WHERE user_id=?", (steal_amount, robber_id))
            await db.execute("UPDATE users SET theft_success = theft_success + 1 WHERE user_id=?", (robber_id,))
            ...
            # Уведомляем жертву
            await bot.send_message(target_id, f"🔫 Вас ограбили! {message.from_user.first_name} украл {steal_amount} монет.")
        else:
            steal_amount = 0
            await db.execute("UPDATE users SET theft_failed = theft_failed + 1 WHERE user_id=?", (robber_id,))
...
async def inventory_handler(message: types.Message):
    ...
    for item_id, name, desc, effect, qty, uses in inv:
        ...
        # Если предмет можно использовать, добавляем кнопку
...
async def finish_giveaway_winners(message: types.Message, state: FSMContext):
    ...
    try:
        async with aiosqlite.connect(DB_PATH) as db:
            ...
            for wid in winners:
                try:
                    await bot.send_message(wid, f"🎉 Поздравляем! Ты выиграл в розыгрыше! Свяжись с админом.")
                except:
                    pass
            await db.commit()
...
async def find_user_result(message: types.Message, state: FSMContext):
    ...
    try:
        if query.startswith('@'):
            username = query[1:]
            async with aiosqlite.connect(DB_PATH) as db:
                async with db.execute("SELECT user_id, first_name, balance, joined_date, theft_attempts, theft_success, theft_failed, theft_protected FROM users WHERE LOWER(username)=?", (username.lower(),)) as cur:
                    row = await cur.fetchone()
        else:
            try:
                uid = int(query)
...
async def block_user_reason(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        try:
            await bot.send_message(uid, f"⛔ Вы заблокированы в боте. Причина: {reason if reason else 'не указана'}")
        except:
            pass
    except Exception as e:
...
async def unblock_user_finish(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        try:
            await bot.send_message(uid, "🔓 Вы разблокированы в боте.")
        except:
            pass
    except Exception as e:
...
async def remove_balance_amount(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        asyncio.create_task(bot.send_message(uid, f"💸 У тебя списано {amount} монет администратором."))
    except Exception as e:
...
async def broadcast_media(message: types.Message, state: FSMContext):
    ...
    await message.answer("Рассылка начата... Это может занять некоторое время.")
    await state.finish()
