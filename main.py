...
# ===== НАСТРОЙКИ =====
BOT_TOKEN = os.getenv("BOT_TOKEN")  # Токен из переменных окружения Railway
...
async def theft_start(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        logging.error(f"Theft start error: {e}")
...
async def theft_target(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        logging.error(f"Theft execution error: {e}")
...
async def select_winner_start(message: types.Message):
    if not await is_admin(message.from_user.id):
        return
    await message.answer("Введи ID розыгрыша:", reply_markup=back_keyboard())
    await SelectWinner.giveaway_id.set()

@dp.message_handler(state=SelectWinner.giveaway_id)
async def select_winner(message: types.Message, state: FSMContext):
    if message.text == "◀️ Назад":
        await state.finish()
        await admin_giveaway_menu(message)
        return
...
async def finish_add_shop_item(message: types.Message, state: FSMContext):
    data = await state.get_data()
    try:
        async with aiosqlite.connect(DB_PATH) as db:
            await db.execute(
...
# ===== РАССЫЛКА (заглушка, можно реализовать позже) =====
